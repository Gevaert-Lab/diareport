...
if ( UP > 0 | DOWN > 0   ) { 
  ...
  cat('Boxplot intensities of the top 20 DE precursors ordered by log2FC')
    ## peptide
    d_l <- data$toptable %>% filter(differential_expressed == 'DOWN') %>% 
      arrange(logFC) %>% head(10)%>%   pull(precursor_id)
    u_l <- data$toptable %>% filter(differential_expressed == 'UP') %>% arrange(logFC) %>% 
       head(10) %>%  pull(precursor_id)
  ...
  ann <-  data$toptable  %>%   filter ( precursor_id  %in% final ) %>% dplyr::select(precursor_id  ,logFC,adjPval) %>%  mutate( Peptide_info=  str_wrap( precursor_id ,width= 15)  ) %>% dplyr::select(  -adjPval)
