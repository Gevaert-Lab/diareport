...
assay(pe[['proteinRS']])[marker$Accession  , ]


assay(pe[['proteinRS']])[marker$Accession  , ] %>% as.data.frame() 
%>% rownames_to_column('protein') %>% tidyr::gather( Sample , summarize_intensities ,- protein) %>%   
  left_join( colData(pe) %>%  as.data.frame() %>% select(SampleName , Group), join_by(Sample == SampleName ))

## be fixed
 ggplot(f, aes(x=Group, y=summarize_intensities, group=protein , color=protein )) + 
geom_pointrange(aes(ymin=summarize_intensities-sd, ymax=summarize_intensities+sd))
