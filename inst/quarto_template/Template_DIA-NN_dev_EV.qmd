...
In the following table, percentage and total number of missing protein value are showed for all the samples.
...
Using an internal annotation database (*ProteinAnnotationsShotList_0.9*), proteins are annotated as intracellular, secreted, transmembrane or other. The number of identified proteins in each category is color-coded by the experimental group.
...
log_info('Running Analysis for each comparison..')
  ...
  for (cmp in names(res_de)) {
    ...
    cat('\n\n')
    cat ('All proteins that are either uniquely present or absent in one of the two groups, and are not included in the differential expression (DE) results, are listed below. A heatmap displayed above the table illustrates the protein intensities across all replicates for the two groups of interest. \n')
