 ...
 type_names <- c('BOVINE','HUMAN')
  ...
  precursor_lists <- lapply(type_names, function(type) {
    as.data.frame(rowData(pe[['precursor_']])) %>%
      filter(str_detect (Protein.Names ,type )) %>%
      pull(Precursor.Id)
  })
...
## relative % 
  ...
  d <- data.frame(
  Sample = names(assay(pe[['precursor_']])[precursor_lists[['BOVINE']], ] %>% base::colSums(na.rm=TRUE))
)
