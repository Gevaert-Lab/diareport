...
#log_info(paste(names(res_de),collapse = ', '))
#log_info(paste(names(res_part_item),collapse = ', '))
#log_info(paste(names(res_part_value),collapse = ', '))
...
log_info('Running Analysis for each comparison..')
  ...
  for (cmp in names(res_de)) {
    ...
    diareport::render_child(data = res_de[[cmp]],
                   pe = pe,
                   sample_rel = res_de[[cmp]]$sample_sel,
                 label = cmp,
                 path= file.path(  params$folder_prj, "Result","Contrast",check_and_substitute_forbidden_chars(cmp)),
    ...
    log_info('Check Absent Analsysis')
    log_info(cmp)
