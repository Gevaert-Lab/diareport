...
if ( UP > 1 | DOWN > 1   ) {
   ...
   cat('Boxplot intensities of the top 12 DE proteins (6 up-regulated, 6 down-regulated) \n')

    d_l <- data$toptable %>% filter(differential_expressed == 'DOWN') %>% arrange(logFC) %>% head(6)%>%   pull(Uniprot_id)
    u_l <- data$toptable %>% filter(differential_expressed == 'UP') %>% arrange(desc(logFC)) %>% head(6) %>%  pull(Uniprot_id)

     final <- c(d_l,u_l)
     
    log_info('Inside Boxplot Protein intensities')

    ab <- as.data.frame(assay(pe[["proteinRS"]])[final,data$sample_sel])
   ...
   ann <-  data$toptable  %>%   filter ( Uniprot_id  %in% final ) %>% 
                  dplyr::select(Uniprot_id ,Genes ,logFC,adjPval) %>%  
                  mutate( Protein_info=  str_wrap( paste(Genes,'\n','LogFC:',round(logFC,2),'adjPval',formatC(adjPval, format = "e", digits = 2) ,sep=' '), width =15) ) %>% 
                  dplyr::select(- Genes, , -adjPval)
   #%>% str_wrap(long_varname, width = var_width)
  #log_info('Before JOIN Sample')

   # tt <- ab %>% rownames_to_column('Protein') %>% left_join(ann, join_by(Protein  == Uniprot_id)) %>%  gather(Sample, Intensity ,- c(Protein ,Protein_info,logFC))
