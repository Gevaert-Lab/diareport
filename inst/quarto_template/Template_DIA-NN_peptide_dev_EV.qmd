---
title: "DE Analysis Report (EV Precursor): `r params$title`"
...
#source('utils_function.R')
 ...
 required_packages = required_packages = c('QFeatures',"ggrepel","tidyr",
                       ...
                       ,"UpSetR",'logger','pander','heatmaply','GGally','yaml','webshot2','htmlwidgets')
...
-   `r if (params$filtering_contaminant) 'Contaminant proteins are removed ' else 'Contaminant proteins are not removed'`
...
-   log2 transformed and then normalized using **`r params$normalization`** approach.

After these steps, the quantitaive features used in the downstream analysis are:

- Number of precursors identified/quantified is : **`{r} dim(assay(pe[['peptideNorm']]))[1]`**
...
limma::plotDensities(assay(pe[["peptideLog"]]),legend=FALSE,main='Precursor log2-tranformed') 
...
The plot shows the completeness of the experiments at precursor and summarized level. On the the x-axis Precursor are ranked based on the increasing NA content.
...
peptidemissingness <-  peptidemissingness + xlab("Precursor index (ranked by data completeness)") +
  theme_bw() +
...
In the following table, total number of and percentace of missing precursor values are showed for all samples. 

```{r %_miss_sample}
DT::datatable( as.data.frame(nNA(pe[["peptideNorm"]])$nNAcols) %>% rename(  Sample_Name = name ) %>% arrange(desc(pNA))  ,
...
In this section, the precursor intensities of 8 EV marker proteins (CD9, CD63, SDCBP, CD8, TSG101, PDCD6IP, FLOT1, GAG-EGFP) are summarized for all groups. The dark dot represents the mean of the sample intensities for each precursor, and the length of the bar corresponds to the interval $\mu \pm \sigma$.
...
peptide_list <-  rowData(pe[['peptideNorm']]) %>% as.data.frame() %>% 
  filter( Protein.Ids %in% marker$Accession ) %>% rownames()

 b <- assay(pe[['peptideNorm']])[peptide_list , ] %>% as.data.frame() 

 b$protein <-   rowData(pe[['peptideNorm']]) %>% as.data.frame() %>% 
  filter( Protein.Ids %in% marker$Accession ) %>% pull(Protein.Ids)
...
f <-  b %>% 
  ...
  tidyr::gather( Sample , summarize_intensities ,- Gene, -protein, -peptide) %>%   
...
log_info('Running Analysis for each comparison..')
  ...
  for (cmp in names(res_de)) {
    ...
    cat ('This volcano plot summarizes the differential expression landscape in the comparison between the groups \n')
    ...
    diareport::render_child(data = res_de[[cmp]],
                 ...
                 template='_templateContrast_Peptide_EV.Rmd') 
    cat('\n\n')
    ...
    if ( dim(res_de[[cmp]]$toptable %>% filter(differential_expressed %in% c('UP','DOWN')) )[1] <= 5 || dim(res_de[[cmp]]$toptable %>% filter(differential_expressed %in% c('UP','DOWN')) )[1]  >=  50     ){
      cat(' Not enough DE precursors (< 5) or too many DE precursors for heatmap visualization \n')
    }else{
    ...
    filt_val <- diareport::select_samples_comparison(contrast_values_parsed, 
                                                     pe , 
                                                     params$contrast)
    ...
    cat('\n\n')
    cat ('All precursor that are either uniquely present or absent in one of the two groups are listed below. Because these precursor are identified exclusively in one group, they are not included in the standard differential expression (DE) results and are therefore listed separately here. The heatmap displayed above the table illustrates the precursor intensities across all replicates for the two groups of interest.\n')
    ...
    cat('\n\n')
    cat('\n\n')
    cat('These bar plots summarize the number of significantly upregulated/downregulated precursor based on different adjusted p-values (selected adjusted p-values are 0.001, 0.01, 0.05, and 0.1 - see facet headers) and log2 fold-change thresholds (on the x-axis) used to define the significance levels')
...
In the table you can find a summary of the number of DE precursors found in all the comparisons. The overlapping of DE precursors among the comparisons is also showed using an upset plot.
