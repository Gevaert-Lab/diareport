...
if ( UP > 1 | DOWN > 1   ) {
   ...
   ann <-  data$toptable  %>%   filter ( Uniprot_id  %in% final ) %>% dplyr::select(Uniprot_id ,Genes ,logFC,adjPval) %>%  mutate( Protein_info=  str_wrap(Genes,width= 10)  ) %>% dplyr::select(- Genes, , -adjPval)
